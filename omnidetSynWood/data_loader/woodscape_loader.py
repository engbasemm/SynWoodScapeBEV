...
class WoodScapeRawDataset(data.Dataset):
    ...
    def __init__(self, data_path=None, path_file=None, is_train=False, config=None):
        ...
        #self.resize  = transforms.Compose([transforms.Resize([224, 224],interpolation=transforms.InterpolationMode.BICUBIC),
        #                                transforms.ToTensor(),
        ...
        #self.resize_label = transforms.Compose([transforms.Resize([224, 224],interpolation=transforms.InterpolationMode.NEAREST),
        #                                transforms.ToTensor(),
        #                                transforms.Lambda(lambda x: x.repeat(3, 1, 1)),
        #                                # gray -> GRB 3 channel (lambda function)
        #                                transforms.Normalize(mean=[0.0, 0.0, 0.0],
        ...
        #if "distance" or "depth" in self.task:
    ...
    def get_detection_label(self, crop_coords, frame_index, cam_side):
        ...
        if os.stat(path).st_size != 0:
            ...
            # --- NEW REMAPPING LOGIC FOR CLASS IDs ---
            # Remap class ID 10 (car) to 0
            boxes[boxes[:, 0] == 10, 0] = 0
            # Remap class ID 4 (person) to 1
            boxes[boxes[:, 0] == 4, 0] = 1
            # --- END NEW REMAPPING LOGIC ---

            newboxes = boxes[torch.where((boxes[:, 0] < self.args.num_classes_detection))[0]]


            #print(len(newboxes) , path)
            if (len(newboxes) == 0)  : #or ( (self.args.discard_mirrors == True) and (cam_side == 'MVL' or cam_side == 'MVR')):
                ...
                boxes = torch.cat( (carboxes , personboxes))
