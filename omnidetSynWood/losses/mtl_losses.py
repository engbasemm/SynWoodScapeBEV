...
class UncertaintyLoss(nn.Module):
    def __init__(self, tasks=None,enable_compression=False):
    ...
    def forward(self, losses):
        ...
        for idx, current_task in enumerate(self.tasks):
            if (current_task == "compression") and (self.enable_compression == False):
                continue
            if (current_task == "compression"):
                if type( losses["compression_loss"]) is dict:
                    losses["compression_loss"] =  losses [ "compression_loss"]["compression_loss"]
