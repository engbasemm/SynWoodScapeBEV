...
def main():
    ...
    if args.train == "bev":
        model = BEVCompressionTrainer(args)
        model.train()
        #model.debug_projections()
    elif args.train == "compression":
        model = CompressionModel(args)
        model.compression_train()
    elif args.train == "depth":
        model = DepthModel(args)
        model.depth_train()
    elif args.train == "distance":
    ...
    elif args.train == "compression_semantic_detection_motion":
        model = CompressionSemanticDetectionMotionModel(args)
        model.compression_semantic_detection_motion_train()
    elif args.train == "compression_semantic_detection":
        model = CompressionSemanticDetectionModel(args)
        model.compression_semantic_detection_train()
    elif args.train == "compression_semantic_detection_bev":
        model = CompressionSemanticDetectionBevModel(args)
        model.compression_semantic_detection_bev_train()
    elif args.train == "distance_compression":
        model = DistanceCompressionModel(args)
        model.distance_compression_train()
    elif args.train == "compression_semantic":
        model = CompressionSemanticModel(args)
        model.compression_semantic_train()
...
#    from train_compress_priors import CompressionPriorModel
    from train_bev_stitching import BEVCompressionTrainer # Import the new focused trainer
    from train_compression_semantic_detection_motion import CompressionSemanticDetectionMotionModel
    from train_compression_semantic_detection import CompressionSemanticDetectionModel
    from train_compression_semantic_detection_bev import CompressionSemanticDetectionBevModel
    from train_depth import DepthModel
    from train_distance_compression import DistanceCompressionModel
