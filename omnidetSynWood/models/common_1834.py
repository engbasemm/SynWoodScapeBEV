...
class resBasicLayer(nn.Module):
    def __init__(self, c1, c2, n=1, s=1, g=1, w=64, downsample=False): #chin, plane, block_nums, group, width_per_group
        super(resBasicLayer,self).__init__()
        blocks=[resBasic(inplanes=c1, planes=c2, stride=s, groups=g, base_width=w, downsample=downsample)]
        for _ in range(n-1):
            blocks.append(resBasic(inplanes=c2*resBasic.expansion, planes=c2, stride=1, groups=g, base_width=w))
