...
from models.compressAI.entropy_models import GaussianConditional
from models.compressAI.priors import get_scale_table, CompressionModel
# from compressai.models import CompressionModel
from models.compressAI.ops import ste_round
from models.compressAI.layers import ResidualBlock, GDN, MaskedConv2d, conv3x3, ResidualBlockWithStride
# from deepspeed.profiling.flops_profiler import get_model_profile
import torch.nn.functional as F
from models.compressAI.ldmic_entropy_model import Hyperprior, CheckMaskedConv2d
...
class EfficientAttention(nn.Module):
    ...
    def parallel_forward(self, target, input):
        ...
        keys = keys.permute(0, 2, 1).reshape(n, h * w, self.head_count, -1).permute(0, 2, 3, 1).reshape(
            n * self.head_count, -1, h * w)
        queries = queries.permute(0, 2, 1).reshape(n, h * w, self.head_count, -1).permute(0, 2, 3, 1).reshape(
            n * self.head_count, -1, h * w)
        values = values.permute(0, 2, 1).reshape(n, h * w, self.head_count, -1).permute(0, 2, 3, 1).reshape(
            n * self.head_count, -1, h * w)
...
class LDMIC(nn.Module):
    ...
    def forward(self, x):
        ...
        y_left_ste, y_right_ste = ste_round(y_left - left_means_hat) + left_means_hat, ste_round(
...
class LDMIC_checkboard(nn.Module):
    ...
    def decompress(self, left_dict, right_dict):
        ...
        y_left, y_right = self.atten_4(self.decoder_1(y_left), self.decoder_1(y_right))
        # print(y_left[0, 0, 0, 0:10], y_right[0, 0, 0, 0:10])
        x_left_hat, x_right_hat = self.decoder_2(y_left), self.decoder_2(
            y_right)  # .clamp_(0, 1), self.decoder_2(y_right).clamp_(0, 1)
...
class SVS_JPC(nn.Module):
    ...
    def forward(self, x, num_camera=None):
        if isinstance(x, list):
            # Original behavior: input is a list of images from different cameras.
            if num_camera is None:
                num_camera = len(x)
            x_cat = torch.cat(x, dim=0)
            y = self.encoder(x_cat)
        elif isinstance(x, torch.Tensor):
            # New behavior for MTL: input is a single, pre-computed (and fused) feature tensor.
            y = x
            if num_camera is None:
                raise ValueError("num_camera must be provided when the input is a single tensor.")
        else:
            raise TypeError(
                f"Invalid input type for Multi_LDMIC. Expected list of tensors or a single tensor, but got {type(x)}")
        ...
        # Resize params to match ctx_params spatial dimensions
...
class Multi_LDMIC_checkboard(nn.Module):
    ...
    def forward_entropy(self, y):
        ...
        # Resize params to match the spatial dimensions of y
        params_resized = F.interpolate(params, size=(y_height, y_width))

        gaussian_params_anchor = self.entropy_parameters(
            torch.cat([ctx_params_anchor, params_resized], dim=1)
        )
        scales_anchor, means_anchor = gaussian_params_anchor.chunk(2, 1)
        ...
        ctx_params_non_anchor = self.context_prediction(anchor)
        gaussian_params_non_anchor = self.entropy_parameters(
            torch.cat([ctx_params_non_anchor, params_resized], dim=1)
        )
        scales_non_anchor, means_non_anchor = gaussian_params_non_anchor.chunk(2, 1)
        scales_hat, means_hat = self.merge(scales_anchor, means_anchor,
                                           scales_non_anchor, means_non_anchor)
...
class OmniDetCompression(nn.Module):
    def __init__(self):
        super().__init__()
        # Add your compression model code here
