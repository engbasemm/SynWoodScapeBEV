...
class YoloDecoder(nn.Module):
    ...
    def forward(self, backbone_features, img_dim, targets=None):
        ...
        # Resize x1_in to match x1's spatial dimensions to handle any size mismatches
        if x1_in.shape[2:] != x1.shape[2:]:
            x1_in = F.interpolate(x1_in, size=x1.shape[2:], mode='bilinear', align_corners=False)
        ...
        # Resize x2_in to match x2's spatial dimensions
        if x2_in.shape[2:] != x2.shape[2:]:
            x2_in = F.interpolate(x2_in, size=x2.shape[2:], mode='bilinear', align_corners=False)
