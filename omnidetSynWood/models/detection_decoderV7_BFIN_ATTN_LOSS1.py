...
# Ensure build_targets is correctly imported from your project elsewhere
try:
...
class YOLOLayer(nn.Module):
    ...
    def _post_process_output_target(self,
                                    ...
                                    prediction_reshaped_logits: torch.Tensor,
                                    decoded_pred_obj_conf_for_nms: torch.Tensor,
        ...
        if targets is None:  # Validation or inference mode
            ...
            # tcls_tgt_full (B,nA,gH,gW,nC)
            # raw_pred_x_obj, ..., raw_pred_obj_conf_obj, raw_pred_cls_obj (1D, N_pos_total_batch)
            ...
            (obj_mask, noobj_mask,
             ...
             # These are full grid tensors
             raw_pred_x_obj, raw_pred_y_obj,
             raw_pred_w_obj, raw_pred_h_obj,
             raw_pred_obj_conf_obj, raw_pred_cls_obj,
             raw_pred_obj_conf_noobj,
             _iou_scores_full, _class_mask_full) = build_targets(  # Suffix _full to indicate they are grid-sized
            ...
            obj_mask = obj_mask.bool()  # Full grid mask (B, nA, gH, gW)

            device = self.args.device
            decoded_pred_boxes_for_loss = torch.empty(0, 4, device=device)
            decoded_target_boxes_for_loss = torch.empty(0, 4, device=device)

            if self.debug_mode:
                print(f"    [DEBUG YOLOLayer _post_process] obj_mask sum for current scale: {obj_mask.sum().item()}")
                if tx_logit_tgt_full is not None:  # Check the full grid target tensor
                    print(
                        f"    [DEBUG YOLOLayer _post_process] tx_logit_tgt_full shape from build_targets: {tx_logit_tgt_full.shape}")

            if obj_mask.any():  # Check if there are any positive samples for this scale in the batch
                # Get raw logits for positive predictions from the reshaped tensor using obj_mask
                raw_x_logits_obj = prediction_reshaped_logits[..., 0][obj_mask]
                raw_y_logits_obj = prediction_reshaped_logits[..., 1][obj_mask]
                raw_w_logits_obj = prediction_reshaped_logits[..., 2][obj_mask]
                raw_h_logits_obj = prediction_reshaped_logits[..., 3][obj_mask]

                # Get corresponding grid cell coords and anchor dims for positive predictions
                ...
                # Decode predicted boxes (cx, cy, w, h in grid scale)
                pred_cx_grid = (torch.sigmoid(raw_x_logits_obj) * 2. - 0.5 + grid_x_obj)
                pred_cy_grid = (torch.sigmoid(raw_y_logits_obj) * 2. - 0.5 + grid_y_obj)
                pred_w_grid = (torch.sigmoid(raw_w_logits_obj) * 2).pow(2) * anchor_w_obj
                pred_h_grid = (torch.sigmoid(raw_h_logits_obj) * 2).pow(2) * anchor_h_obj
                decoded_pred_boxes_for_loss = torch.stack([pred_cx_grid, pred_cy_grid, pred_w_grid, pred_h_grid], dim=1)

                # Decode target boxes (cx, cy, w, h in grid scale)
                # Use obj_mask to select positive samples from the full grid target logit tensors
                tx_logit_tgt_obj = tx_logit_tgt_full[obj_mask]
                ty_logit_tgt_obj = ty_logit_tgt_full[obj_mask]
                tw_logit_tgt_obj = tw_logit_tgt_full[obj_mask]
                th_logit_tgt_obj = th_logit_tgt_full[obj_mask]

                target_cx_grid = (torch.sigmoid(tx_logit_tgt_obj) * 2. - 0.5 + grid_x_obj)
                target_cy_grid = (torch.sigmoid(ty_logit_tgt_obj) * 2. - 0.5 + grid_y_obj)
                target_w_grid = (torch.sigmoid(tw_logit_tgt_obj) * 2).pow(2) * anchor_w_obj
                target_h_grid = (torch.sigmoid(th_logit_tgt_obj) * 2).pow(2) * anchor_h_obj
                decoded_target_boxes_for_loss = torch.stack(
                    [target_cx_grid, target_cy_grid, target_w_grid, target_h_grid], dim=1)

                if self.debug_mode:
                    print(f"        [DEBUG YOLOLayer _post_process] INSIDE obj_mask.any():")
                    print(
                        f"            decoded_pred_boxes_for_loss shape: {decoded_pred_boxes_for_loss.shape}, e.g., {decoded_pred_boxes_for_loss[0].tolist() if decoded_pred_boxes_for_loss.numel() > 0 else 'N/A'}")
                    print(
                        f"            decoded_target_boxes_for_loss shape: {decoded_target_boxes_for_loss.shape}, e.g., {decoded_target_boxes_for_loss[0].tolist() if decoded_target_boxes_for_loss.numel() > 0 else 'N/A'}")

            # Dictionaries for the loss function
            # Ensure tcls_tgt_flat is used if build_targets returns it as 1D, otherwise mask tcls_tgt_full
            # Based on your build_targets, raw_pred_cls_obj and tcls_tgt_flat are 1D (N_pos, nC)
            target_dict_for_loss = dict(
                decoded_target_boxes_obj=decoded_target_boxes_for_loss,
                ...
                no_obj_conf=torch.zeros_like(
                    raw_pred_obj_conf_noobj) if raw_pred_obj_conf_noobj.numel() > 0 else torch.empty(0, device=device),
                cls=tcls_tgt_flat,  # Use the 1D filtered tcls from build_targets
                obj_mask=obj_mask,
                noobj_mask=noobj_mask
            )
            output_dict_for_loss = dict(
                decoded_pred_boxes_obj=decoded_pred_boxes_for_loss,
                obj_conf=raw_pred_obj_conf_obj,  # 1D raw logits for positive samples
                no_obj_conf=raw_pred_obj_conf_noobj,  # 1D raw logits for negative samples
                cls=raw_pred_cls_obj,  # 1D raw logits for positive samples
            )
    ...
    def forward(self, x: torch.Tensor, targets: Optional[torch.Tensor] = None,
        ...
        return self._post_process_output_target(targets, raw_predictions_from_conv, prediction_reshaped_logits,
                                                decoded_pred_obj_conf_nms, current_grid_shape)
...
class YoloDecoder(nn.Module):
    ...
    def forward(self, backbone_features: List[torch.Tensor], img_dim: Optional[Union[List[int], torch.Tensor]] = None,
        ...
        for i in range(self.num_scales):
            nms_output, output_dict_loss_scale, target_dict_loss_scale = self.final_layers[i](
                neck_outs[i], targets, current_img_dim_val
            )
            if self.debug_mode and targets is not None:
                print(
                    f"  [DEBUG YoloDecoder forward] Scale {i} - output_dict_loss_scale from YOLOLayer IS NONE: {output_dict_loss_scale is None}")
                if output_dict_loss_scale is not None:
                    print(
                        f"  [DEBUG YoloDecoder forward] Scale {i} - output_dict_loss_scale keys: {list(output_dict_loss_scale.keys())}")
                    pred_boxes_tensor = output_dict_loss_scale.get('decoded_pred_boxes_obj')
                    print(
                        f"  [DEBUG YoloDecoder forward] Scale {i} - decoded_pred_boxes_obj shape: {pred_boxes_tensor.shape if pred_boxes_tensor is not None else 'None'}")
                print(
                    f"  [DEBUG YoloDecoder forward] Scale {i} - target_dict_loss_scale from YOLOLayer IS NONE: {target_dict_loss_scale is None}")
                if target_dict_loss_scale is not None:
                    print(
                        f"  [DEBUG YoloDecoder forward] Scale {i} - target_dict_loss_scale keys: {list(target_dict_loss_scale.keys())}")
                    tgt_boxes_tensor = target_dict_loss_scale.get('decoded_target_boxes_obj')
                    print(
                        f"  [DEBUG YoloDecoder forward] Scale {i} - decoded_target_boxes_obj shape: {tgt_boxes_tensor.shape if tgt_boxes_tensor is not None else 'None'}")

            if nms_output is not None: yolo_nms_outputs_list.append(nms_output)
            yolo_output_dicts_for_loss.append(output_dict_loss_scale if output_dict_loss_scale is not None else {})
            yolo_target_dicts_for_loss.append(target_dict_loss_scale if target_dict_loss_scale is not None else {})
