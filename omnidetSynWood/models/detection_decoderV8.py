...
class Conv(nn.Module):
   # Standard convolution with BatchNorm and ReLU activation
...
class SPPCSPC(nn.Module):
   ...
   def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5,9,13)):
       ...
       c_ = int(2 * c2 * e)  # hidden channels for internal convolutions
...
class E_ELAN(nn.Module):
   ...
   def __init__(self, in_ch, out_ch, e=0.5):
       ...
       split = hidden // 2 # channels for the split branches (output of initial split convs)
       ...
       # Channels: hidden (from x1) + split*4 (from x3, x5, x7, x9)
       self.cv10 = Conv(hidden + split * 4, out_ch, 1, 1) # Final conv to output channels (hidden + 2*hidden -> out_ch)
...
class RepConv(nn.Module):
   ...
   def __init__(self, c1, c2, k=3, s=1, p=1, g=1, act=True, deploy=False):
       ...
       if deploy:
           ...
           # Linear branch (1x1 conv with padding to match spatial size)
           # Removed 'p' argument for 1x1 conv as it's not needed and causes TypeError
   ...
   def _fuse_kernel(self):
       ...
       if hasattr(self, 'rconv_bn'): # Check if in training mode
           ...
           self.rconv = nn.Conv2d(self.in_channels, self.out_channels,
                                  self.rconv.kernel_size, self.rconv.stride, self.rconv.padding,
                                  groups=self.groups, bias=True).to(fused_kernel.device)
           self.rconv.weight.data = fused_kernel
           self.rconv.bias.data = fused_bias
...
class YOLOLayer(nn.Module):
   ...
   def _post_proccess_output_target(self, targets):
       ...
       if targets is None:
           ...
           # The current YOLOLayer passes decoded/sigmoid predictions (self.pred_boxes, self.pred_cls).
   ...
   def forward(self, x, targets=None, img_dim=None):
       ...
       prediction = (prediction.view(num_samples, self.num_anchors, self.num_classes + num_of_output,
       ...
       # Use torch.softmax for class probabilities.
       # Use torch.exp for width and height scaling.


       # Apply sigmoid to x, y, and objectness confidence
       self.x = torch.sigmoid(prediction[..., 0]) # Decoded x (relative to cell, range 0-1)
       self.y = torch.sigmoid(prediction[..., 1]) # Decoded y (relative to cell, range 0-1)
       self.pred_conf = torch.sigmoid(prediction[..., 4]) # Objectness confidence (Sigmoid)


       # Apply softmax to class probabilities
       self.pred_cls = torch.softmax(prediction[..., 5:], dim=-1) # Class probabilities (Softmax over class dimension)
       ...
       self.pred_boxes = torch.empty(prediction[..., :4].shape).to(self.args.device) # Initialize tensor
       ...
       # Need access to the raw prediction tensor 'prediction' here.


       # Decoded x, y (relative to cell, range -0.5 to 1.5 after *2-0.5) + grid offset
       self.pred_boxes[..., 0] = (torch.sigmoid(prediction[..., 0]) * 2. - 0.5 + self.grid_x)
       self.pred_boxes[..., 1] = (torch.sigmoid(prediction[..., 1]) * 2. - 0.5 + self.grid_y)
       ...
       self.pred_boxes[..., 2] = (torch.sigmoid(prediction[..., 2]) * 2).pow(2) * self.anchor_w
       self.pred_boxes[..., 3] = (torch.sigmoid(prediction[..., 3]) * 2).pow(2) * self.anchor_h
       ...
       # Call the post-processing/target building method
...
class YoloDecoder(nn.Module):
   ...
   def __init__(self, _out_filters, args):
       ...
       Args:
           _out_filters (list): List of input channel dimensions from the backbone.
                               This decoder is designed for 3 input feature maps (P3, P4, P5).
                               Assumed order: [channels_P3, channels_P4, channels_P5].
       ...
       # Store input channel dimensions from the backbone
       # Assuming _out_filters contains channels for P3, P4, P5 in order
       self.in_channels = _out_filters[-3:] # Use the last 3 feature map channels (P3, P4, P5)


       # Number of scales (detection heads)
       self.num_scales = 3 # Hardcoded for 3 detection heads (P3, P4, P5)
       ...
       # Convert Tupperware objects to lists before creating the tensor
       # Assuming anchors1 -> P5 (largest stride), anchors2 -> P4 (medium stride), anchors3 -> P3 (smallest stride)
       # based on the original code's final layer assignments.
       anchors_list = [list(args.anchors1), list(args.anchors2), list(args.anchors3)]


       # Assuming same number of anchors per scale, based on the provided YAML structure
       # Use the length of the first anchor list after conversion
       ...
       # This section replaces the original _make_embedding blocks with YOLOv7-style modules.
       # The neck processes feature maps from P5, P4, and P3, building a feature pyramid.


       # P5 path (Largest stride, smallest spatial size): SPPCSPC -> Conv -> Output for P5 head and branch for P4 path
       # Input channels: self.in_channels[-1] (channels of P5 from backbone)
       # Output channels of SPPCSPC: Set to match the expected input of the next E-ELAN path (P4 path).
       # The branch from this path feeds into the P4 path via concatenation.
       # Based on the original structure, the branch feeding into embedding1 had 256 channels.
       ...
       self.sppcspc = SPPCSPC(self.in_channels[-1], sppcspc_branch_out_channels)


       # A convolutional layer after SPPCSPC to produce the input for the P5 detection head.
       # Input channels: Output channels of SPPCSPC (sppcspc_branch_out_channels)
       # Output channels: Define the input channels for the P5 detection head.
       # Let's use a standard size like 512 for the head input channels.
       p5_head_in_channels = 512 # Input channels for the P5 detection head
       self.conv_p5_head_in = Conv(sppcspc_branch_out_channels, p5_head_in_channels, k=1)
       ...
       # Input to concatenation: upsampled SPPCSPC branch (sppcspc_branch_out_channels) and p4_in (self.in_channels[-2])
       # Input channels to E-ELAN P4: sppcspc_branch_out_channels + self.in_channels[-2]
       # Output channels of E-ELAN P4: Define the input channels for the P4 detection head.
       # Let's use a standard size like 512 for the head input channels.
       p4_head_in_channels = 512 # Input channels for the P4 detection head


       e_elan_p4_in = sppcspc_branch_out_channels + self.in_channels[-2]
       e_elan_p4_out = p4_head_in_channels # Output channels of E-ELAN P4


       # E-ELAN module for the P4 path. Intermediate channels can be tuned.
       # Using a simple split factor based on input channels for intermediate layers.
       # Simplified E_ELAN constructor used here.
       self.e_elan_p4 = E_ELAN(e_elan_p4_in, e_elan_p4_out, e=0.5)
       ...
       # Output channels: Set to match the expected input of the next E-ELAN path (P3 path).
       # The branch from this path feeds into the P3 path via concatenation.
       # Based on the original structure, the branch feeding into embedding2 had 128 channels.
       ...
       # Input to concatenation: upsampled E-ELAN P4 branch (e_elan_p4_branch_out_channels) and p3_in (self.in_channels[-3])
       # Input channels to E-ELAN P3: e_elan_p3_in = e_elan_p4_branch_out_channels + self.in_channels[-3]
       # Output channels of E-ELAN P3: Define the input channels for the P3 detection head.
       # Let's use a standard size like 256 for the head input channels.
       p3_head_in_channels = 256 # Input channels for the P3 detection head


       e_elan_p3_in = e_elan_p4_branch_out_channels + self.in_channels[-3]
       e_elan_p3_out = p3_head_in_channels # Output channels of E-ELAN P3


       # E-ELAN module for the P3 path. Intermediate channels can be tuned.
       # Using a simple split factor based on input channels for intermediate layers.
       # Simplified E_ELAN constructor used here.
       self.e_elan_p3 = E_ELAN(e_elan_p3_in, e_elan_p3_out, e=0.5)




       # --- Define Final YOLOLayers (Detection Heads) ---
       # These layers process the output of the neck (E-ELAN/Conv outputs) and produce the final predictions.
       # Each YOLOLayer now includes a simplified head structure with RepConv layers.
       # We pass the determined input channel count for each head during initialization.


       # P5 Detection Head (Largest stride) - Receives output from conv_p5_head_in
       # Assuming anchors1 corresponds to P5 (largest stride)
       ...
       # Set grid size and compute offsets for bias initialization
       # The grid sizes for the final layers should match the order of the final layers (P5, P4, P3)
       # So, final_layer0 (P5) uses grid_sizes[0]
       self.final_layer0.compute_grid_offsets(self.grid_sizes[0])




       # P4 Detection Head (Medium stride) - Receives output from e_elan_p4
       # Assuming anchors2 corresponds to P4 (medium stride)
       ...
       # Set grid size and compute offsets for bias initialization
       # final_layer1 (P4) uses grid_sizes[1]
       self.final_layer1.compute_grid_offsets(self.grid_sizes[1])




       # P3 Detection Head (Smallest stride) - Receives output from e_elan_p3
       # Assuming anchors3 corresponds to P3 (smallest stride)
       ...
       # Set grid size and compute offsets for bias initialization
       # final_layer2 (P3) uses grid_sizes[2]
       self.final_layer2.compute_grid_offsets(self.grid_sizes[2])
       ...
       # Correcting the order to match the neck outputs: P3, P4, P5
       ...
       # Initialize the bias of the final prediction layers to improve training convergence.
       # This is called after compute_grid_offsets has been called for each head.
       self._init_biases()
   ...
   def _init_biases(self):
       ...
       # Initialize bias for objectness to a small positive value to encourage low initial objectness score.
       # Initialize bias for class probabilities based on expected class frequency (not implemented here).
       # The formula math.log(8 / (stride_w * stride_h)) is a common heuristic based on anchor size and stride.
       # Use the strides stored in each head instance.
       # The order here should match the order of final_layers (P3, P4, P5)
       yolo_layers = self.final_layers # Use the ModuleList directly


       for i, head in enumerate(yolo_layers):
           conv = head.prediction
           # Use .data to perform in-place operation without affecting the gradient graph
           # Reshape bias data to (num_anchors, 5 + num_classes)
           b = conv.bias.data.view(head.num_anchors, -1) # Use head.num_anchors for clarity
           # Add bias to the objectness score (index 4)
           # Use the head's stride for the calculation
           head_stride_w = head.stride[0]
           head_stride_h = head.stride[1]
           # Ensure strides are not zero before division (should be handled by compute_grid_offsets now)
           if head_stride_w > 0 and head_stride_h > 0:
                # Using the standard YOLOv7 bias initialization based on prior
                prior = 0.01 # A common prior for objectness
                bias_value = -math.log((1 - prior) / prior) # log(0.01 / 0.99) approx -4.595
                b[:, 4] = bias_value
                # print(f"Initialized objectness bias for YOLOLayer {i} (stride W:{head_stride_w}, H:{head_stride_h}) to {bias_value:.4f}")
           else:
               print(f"Warning: Zero stride encountered in _init_biases for head with grid size {head.grid_size}. Bias not initialized for objectness.")
           # No need to assign back to conv.bias as we modified conv.bias.data directly
   ...
   def switch_to_deploy(self):
       ...
       for module in self.modules():
           # Check if the module is an instance of RepConv and not already in deploy mode
   ...
   def forward(self, backbone_features, img_dim, targets=None):
       ...
       Args:
           backbone_features (list): List of feature maps from the backbone.
                                     Assumed to be ordered from smallest stride (largest spatial)
                                     to largest stride (smallest spatial).
                                     We use the last three features (P3, P4, P5).
       ...
       """
       # backbone_features is a list of feature maps from the backbone.
       # Assuming it's ordered from smallest to largest spatial size (P3, P4, P5).
       # We need the last three feature maps for the decoder.
       # p3_in: smallest stride (largest spatial), p4_in: medium, p5_in: largest stride (smallest spatial)
       p3_in, p4_in, p5_in = backbone_features[-3:] # Assuming order P3, P4, P5
       ...
       # Use self.final_layers which is already in P3, P4, P5 order
       ...
       # --- Add Grid Shapes to Output Dictionaries ---
       ...
       for i in range(len(yolo_output_dict_list)):
           if yolo_output_dict_list[i] is not None:
               yolo_output_dict_list[i]['grid_shapes'] = grid_shapes_list
