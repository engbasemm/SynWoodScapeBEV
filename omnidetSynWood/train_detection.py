...
#from models.detection_decoderV7_BFIN_ATTN import YoloDecoder
...
class DetectionModelBase(TrainUtils):
    def __init__(self, args):
        ...
        self.best_mAP = 0
        self.validation_conf_thres = self.args.detection_conf_thres
    ...
    def detection_val(self, iou_thres, conf_thres, nms_thres, img_size):
        ...
        for batch_idx, inputs in enumerate(self.val_loader):
            ...
            # In detection_val, after NMS call:
            if self.args.debug_decoder:
                num_dets_after_nms_per_image = [len(dets) if dets is not None else 0 for dets in outputs]
                print(
                    f"[DEBUG detection_val] Batch {batch_idx} - Detections after NMS (per image): {num_dets_after_nms_per_image}, Total for batch: {sum(num_dets_after_nms_per_image)}")
            sample_metrics += get_batch_statistics(outputs, targets, iou_threshold=iou_thres, args=self.args)
            if batch_idx == 0:
                self.logs["input"] = inputs[("color", 0, 0, self.val_loader.dataset.all_cam_sides[0])] # Log one view
    ...
    def save_best_detection_weights(self):
        ...
        precision, recall, AP, f1, ap_class = self.detection_val(iou_thres=0.5,
                                                                 conf_thres=self.validation_conf_thres,
                                                                 nms_thres=0.2,
