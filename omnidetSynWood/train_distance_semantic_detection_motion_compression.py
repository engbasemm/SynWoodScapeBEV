...
class DistanceSemanticDetectionMotionCompressionModel(DistanceSemanticDetectionMotionModel, CompressionModel):
    ...
    def distance_semantic_detection_motion_compression_train(self):
        ...
        for self.epoch in range(self.args.epochs):
            ...
            for batch_idx, inputs in enumerate(self.train_loader):
                ...
                outputs, losses = self.distance_semantic_detection_motion_compression_loss_predictions(inputs , mode='val')
    ...
    def distance_semantic_detection_motion_compression_loss_predictions(self, inputs, mode='train'):
        features, outputs, losses = self.distance_semantic_detection_motion_loss_predictions(inputs, mode)
        # Note: We are taking features passed through encoder when the dataset split for all the tasks is same
        detection_outputs, detection_losses = self. predict_c(inputs, outputs, features=features)
